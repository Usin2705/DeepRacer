...
class MyCar:
    ...
    def reward_function(self, params):
        ...
        # Reward for complete the race
        if progress >= 100:
            reward = 100
        
        # Print the parameter out to console (so we can see it in local training)                
        carString=('R: {:5.1f}, PRO: {:5.2f}, PRE_PRO: {:5.2f}, '
                   'PRE_PRO2: {:5.2f}, TURN:{:3.0f}, SPE: {:3.1f}, '
                   'IS_LEFT {:d}, NORMDIST: {:.2f}, DISTANCE: {:.2f}, '
                   ' STEP:{:4.0f}'.format(reward, progress, self.pre_progress, 
                   self.pre_progress2, steering_angle, speed, 
                   is_left_of_center, normDistance, distance_from_center, 
                   steps))
        print(carString)        
